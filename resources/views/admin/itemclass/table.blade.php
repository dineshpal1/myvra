                ...
                <table>
                    <thead>
                        <tr class="table100-head">
                            ...
                            <th class="column4"
                                onclick="filter('{{route('item_class-list',['1','1'])}}','table-list','1','class_name','{{ $orderBy == 'desc'?'asc':'desc'}}',$('#textsearch').val())">
                                Class Name
                                @if($argOrderIn == 'class_name')
                                @if($orderBy == 'desc')
                            ...
                            <th class="column4"
                                onclick="filter('{{route('item_class-list',['1','1'])}}','table-list','1','created_at','{{ $orderBy == 'desc'?'asc':'desc'}}',$('#textsearch').val())">
                                Created At
                            ...
                            <th class="column4"
                                onclick="filter('{{route('item_class-list',['1','1'])}}','table-list','1','updated_at','{{ $orderBy == 'desc'?'asc':'desc'}}',$('#textsearch').val())">
                                Updated At
                            ...
                            <th class="column4"
                                onclick="filter('{{route('item_class-list',['1','1'])}}','table-list','1','is_active','{{ $orderBy == 'desc'?'asc':'desc'}}',$('#textsearch').val())">
                                Status
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="column4">
                                <a href="#"
                                    class="name">{{$objData->class_name}}</a>
                                <a href="{{route('item_class/edit',['id'=>$objData->id])}}"
                                    class="d-inline-block text-success mr-3 edit-icon"
                                    data-toggle="tooltip" data-placement="top"
                                    title="Edit" style="color: #9e9e9e!important;"><i
                                        class="bx bx-edit"></i></a>
                                <a href="javascript:void(0)"
                                    onclick="deleteRequest('{{route('item_class/delete',['id'=>$objData])}}',{{$objData->id}},'{{route('item_class-list', ['1'])}}','table-list','1')"
        ...
        <ul class="pagination">
            <li>
                <button class="prev" {{ $argSlot > 1 ? "" : "disabled"}}
                    onclick="getTableData('{{route('item_class-list',[(($varTotalNoOfPages * ($argSlot - 1)) - ($varTotalNoOfPages) + 1), ($argSlot - 1), $argSearchValue])}}','table-list','1')"
            ...
            @for ($i = 1; $i <= $varPaginationCount; $i++) @php $varPageNo=($i +
                ...
                <button type="button" class="{{$varActiveClass}}"
                    onclick="getTableData('{{route('item_class-list',[$varPageNo, $argSlot, $argSearchValue])}}','table-list','1')"
                ...
                <li>
                    <button class="next"
                        ...
                        title="next page"
                        onclick="getTableData('{{route('item_class-list',[(($varTotalNoOfPages * ($argSlot)) + 1), ($argSlot + 1), $argSearchValue])}}','table-list','1')">&#10095;</button>
                </li>
        </ul>
    </div>
    @else
